    ...
    <div className="print-document-root font-cairo bg-white text-slate-900">
      {pages.map((page, index) => {
        ...
        return (
          ...
          >
            {/* ========================================================================= */}
            {/* 1. OFFICIAL SCHOOL HEADER (TOP OF WORKSHEET) */}
            {/* ========================================================================= */}
            {/* 1. DOCUMENT HEADER */}
            {/* ========================================================================= */}
            {!isCover && (
              <header className="border-b-2 border-emerald-950 pb-2.5 mb-3 space-y-1.5">
                <div className="flex items-center justify-between gap-3">
                  {/* Right: Country, Ministry, School Name and Department */}
                  <div className="flex items-center gap-2.5">
                    ...
                    <div className="text-right">
                      <div className="text-[10px] font-black text-emerald-800">
                        {branding.countryName || 'الجمهورية اليمنية'} — {branding.ministryName || 'وزارة التربية والتعليم'}
                      </div>
                  ...
                  {/* Left: Page Number & Grade Badge */}
                  <div className="text-left shrink-0">
                    <div className="inline-flex items-center justify-center w-9 h-9 rounded-xl bg-slate-900 text-white font-black text-sm">
                      {page.pageNumber}
                    </div>
                ...
                {branding.showStudentInfoInPrint && (
                  <div className="grid grid-cols-4 gap-2 pt-1.5 border-t border-slate-300 text-[11px] bg-slate-50 px-2.5 py-1 rounded-lg">
                    <div>
                      <span className="font-bold text-slate-600">اسم التلميذ/ة: </span>
                      <span className="font-black text-slate-900">{studentName || branding.studentName || '................................'}</span>
                    </div>
                    <div>
                      <span className="font-bold text-slate-600">الصف: </span>
                      <span className="font-bold text-slate-900">{studentGrade || branding.studentGrade || 'الأساسي'}</span>
                    </div>
                    <div>
                      <span className="font-bold text-slate-600">الشعبة / الفصل: </span>
                      <span className="font-bold text-slate-900">{studentClass || branding.studentClass || 'شعبة أ'}</span>
                    </div>
                    <div>
                      <span className="font-bold text-slate-600">التاريخ: </span>
                      <span className="font-bold text-slate-900">.... / .... / ١٤٤هـ</span>
                    </div>
                  </div>
                )}
              </header>
            ...
            <main className="flex-1 space-y-4">
              ...
              {!isCover && !isEvaluation && !isToc && (
                <div className="space-y-4">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="grid grid-cols-7 gap-2">
                        {page.content?.gridItems?.map((group, idx) => (
                          <div key={idx} className="border-2 border-slate-800 rounded-xl p-2 text-center bg-white">
                            <span className="text-2xl font-black block text-slate-900 mb-1">{group}</span>
                  ...
                  {/* WRITTEN TRACING & COPYING EXERCISES (تدريبات النسخ والتتبع الثلاثية) */}
                  {page.pageType === 'written_tracing' && (
                    <div className="space-y-2.5">
                      <div className="bg-slate-100 border border-slate-400 rounded-lg p-1.5 flex items-center justify-between text-xs font-black text-slate-900">
                        <span>تدريب كتابي: اقرأ النموذج ➜ تتبع المقطع المنقط بقلم الرصاص ➜ اكتب المقطع بمفردك:</span>
                      </div>
                        ...
                        {page.content?.gridItems?.map((item, idx) => (
                          <div key={idx} className="border-2 border-slate-800 rounded-xl p-2 bg-white flex flex-col gap-1">
                            <div className="grid grid-cols-3 gap-1.5 text-center items-stretch">
                              {/* 1. Original Syllable */}
                              <div className="border border-slate-300 bg-slate-50 rounded-lg p-1.5 flex flex-col justify-between min-h-[62px]">
                                <span className="text-[9px] text-slate-500 font-bold">النموذج</span>
                              ...
                              <div className="border border-dashed border-slate-400 bg-slate-50/50 rounded-lg p-1.5 flex flex-col justify-between min-h-[62px]">
                                <span className="text-[9px] text-slate-500 font-bold">تتبع منقط</span>
                                <span className="arabic-dotted-tracing text-2xl font-black">
                                  {item}
                                </span>
                              </div>

                              {/* 3. Empty Writing Box */}
                              <div className="border-2 border-slate-700 bg-white rounded-lg p-1.5 flex flex-col justify-between min-h-[62px]">
                                <span className="text-[9px] text-slate-500 font-bold">اكتب هنا</span>
                                <div className="w-full h-4 border-b-2 border-slate-700 border-dotted mt-1"></div>
                              </div>
                            </div>
                          </div>
                        ))}
                  ...
                  {page.pageType === 'dictation_board' && (
                    <div className="space-y-3">
                      <div className="bg-slate-100 border border-slate-400 p-2.5 rounded-xl text-center">
                        ...
                        <p className="text-xs text-slate-600 font-bold">استمع للمعلم واكتب الكلمة بدقة في الخانة المخصصة ثم اضبط حركاتها:</p>
                      </div>

                      <div className="grid grid-cols-2 gap-2.5">
                        {Array.from({ length: Math.min(10, Math.max(8, page.content?.dictationSuggestedWords?.length || 8)) }).map((_, dIdx) => (
                          <div key={dIdx} className="border-2 border-slate-800 rounded-xl p-2.5 bg-white flex items-center justify-between">
                            ...
                            <div className="w-40 h-5 border-b-2 border-slate-700 border-dotted"></div>
                            <div className="border border-slate-300 px-1.5 py-0.5 rounded text-[10px] text-slate-500 font-bold">
                              [ ] صحيح
                            </div>
                          </div>
                        ))}
                        ...
                        <div className="mt-3 p-2.5 bg-slate-50 border border-dashed border-slate-400 rounded-xl">
                          ...
                          <div className="flex flex-wrap gap-1.5">
                            {page.content.dictationSuggestedWords.map((w, wIdx) => (
                              <span key={wIdx} className="bg-white border border-slate-300 px-2 py-0.5 rounded text-xs font-black text-slate-900">
                                {w}
                              </span>
                            ))}
                          </div>
                    ...
                    <div className="space-y-3">
                      <div className={`grid gap-2.5 ${
                        ...
                        {page.content?.gridItems?.map((word, idx) => (
                          <div 
                            key={idx} 
                            className="border-2 border-slate-800 rounded-xl p-2.5 text-center flex flex-col justify-between min-h-[64px] bg-white"
                          >
                            <span className="text-xl sm:text-2xl font-black text-slate-950 font-amiri block">
                              {word}
                            </span>

                            {/* Handwriting Line below the word */}
                            {branding.showHandwritingLinesInPrint && (
                              <div className="w-full pt-1 mt-1 border-t border-dashed border-slate-300 flex flex-col justify-end">
                                <div className="h-4 border-b-2 border-slate-400 border-dotted"></div>
                              </div>
                            )}
                          </div>
                        ))}
                   ...
                   page.content?.gridItems && (
                    <div className="space-y-3">
                      <div className="grid grid-cols-3 sm:grid-cols-4 gap-2.5">
                        {page.content.gridItems.map((word, idx) => (
                          <div key={idx} className="border-2 border-slate-800 rounded-xl p-2.5 text-center bg-white">
                            <span className="text-xl font-black text-slate-950 font-amiri block">{word}</span>
                            {branding.showHandwritingLinesInPrint && (
                              <div className="w-full pt-1 mt-1 border-t border-dashed border-slate-300">
                                <div className="h-4 border-b-2 border-slate-400 border-dotted"></div>
                              </div>
                            )}
                          </div>
                  ...
                  {page.pageType === 'connect_and_read' && (
                    <div className="space-y-2.5">
                      <div className="bg-slate-100 border border-slate-400 rounded-lg p-1.5 text-xs font-black text-slate-900">
                        صل الحروف المنفصلة ➜ تتبع الكلمة المنقطة بقلم الرصاص ➜ اكتب الكلمة كاملة بخط جميل:
                      </div>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                        {page.content?.connectExercises?.map((ex, idx) => (
                          <div key={idx} className="border-2 border-slate-800 rounded-xl p-2.5 flex items-center justify-between gap-2 bg-white">
                            <div className="flex items-center gap-1.5 text-sm font-bold">
                              <span className="bg-slate-100 px-2 py-1 rounded border border-slate-300 font-amiri font-bold text-base">{ex.separated}</span>
                              <span className="text-slate-400">➜</span>
                            </div>
                            <div className="border border-dashed border-slate-400 bg-slate-50 px-2 py-0.5 rounded text-center">
                              <span className="text-[9px] text-slate-500 block">تتبع</span>
                              <span className="arabic-dotted-tracing text-xl font-black">{ex.combined}</span>
                            </div>
                            <div className="border-2 border-slate-700 rounded px-2 py-1 flex flex-col justify-between min-w-[70px]">
                              <span className="text-[9px] text-slate-500 block text-center">اكتب</span>
                              <div className="h-4 border-b-2 border-slate-700 border-dotted"></div>
                            </div>
                          </div>
                        ))}
                      </div>
                    ...
                    <div className="space-y-2.5">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                        {page.content?.analysisWords?.map((item, idx) => (
                          <div key={idx} className="border-2 border-slate-800 rounded-xl p-2.5 space-y-2 bg-white">
                            <div className="flex items-center justify-between border-b border-slate-200 pb-1">
                              <span className="text-xl font-black text-slate-950 font-amiri">{item.word}</span>
                              <div className="flex items-center gap-1">
                                {item.syllables.map((syl, sIdx) => (
                                  <span key={sIdx} className="border border-dashed border-slate-400 bg-slate-50 px-2 py-0.5 rounded text-xs font-black arabic-dotted-tracing">
                                    {syl}
                                  </span>
                                ))}
                              </div>
                            ...
                            <div className="flex items-center justify-between gap-1 pt-0.5">
                              <span className="text-[10px] text-slate-500 font-bold">كتابة الطالب:</span>
                              <div className="flex items-center gap-1.5 flex-1 justify-end">
                                {item.syllables.map((_, sIdx) => (
                                  <div key={sIdx} className="w-10 h-6 border-2 border-slate-700 rounded bg-white flex items-end justify-center pb-0.5">
                                    <div className="w-6 border-b border-slate-400 border-dotted"></div>
                                  </div>
                                ))}
                              </div>
                            </div>
                  ...
                  {page.pageType === 'sentences_reading' && (
                    <div className="space-y-2.5">
                      <p className="text-xs font-bold text-slate-700">اقرأ الجمل التالية بطلاقة ثم اكتبها بخطك الجميل:</p>
                      {page.content?.sentences?.map((sentence, idx) => (
                        <div key={idx} className="border-2 border-slate-800 rounded-xl p-3.5 space-y-2 bg-white">
                          <div className="text-lg font-black text-slate-950 font-amiri">
                            {sentence}
                          </div>
                          {branding.showHandwritingLinesInPrint && (
                            <div className="pt-2 border-t border-slate-300 space-y-1">
                              <div className="h-5 border-b-2 border-slate-500 border-dotted"></div>
                            </div>
                          )}
                        </div>
                      ))}
            ...
            {/* ========================================================================= */}
            {/* 3. OFFICIAL TEACHER EVALUATION BOX & FOOTER */}
            {/* ========================================================================= */}
            {!isCover && (
              <footer className="mt-4 pt-2 border-t-2 border-slate-800 space-y-2">
                {/* Teacher Evaluation & Signatures */}
                {branding.showEvaluationBoxInPrint && (
                  <div className="grid grid-cols-3 gap-2 bg-slate-50 border border-slate-400 p-2 rounded-xl text-[11px]">
                    <div className="flex items-center gap-2">
                      <span className="font-black text-slate-900">تقييم الأداء:</span>
                      <span className="border border-slate-400 px-1.5 py-0.5 rounded bg-white font-bold">[ ] متقن ⭐</span>
                      <span className="border border-slate-400 px-1.5 py-0.5 rounded bg-white font-bold">[ ] يحتاج تكرار</span>
                    </div>
                    <div>
                      <span className="font-bold text-slate-700">توقيع المعلم: </span>
                      <span className="font-bold text-slate-900">........................</span>
                    </div>
                    <div>
                      <span className="font-bold text-slate-700">توقيع ولي الأمر: </span>
                      <span className="font-bold text-slate-900">........................</span>
                    </div>
                  </div>
                )}
                ...
                <div className="flex items-center justify-between text-[10px] text-slate-600 font-bold px-1">
                  <span>🇾🇪 {branding.countryName || 'الجمهورية اليمنية'} — {branding.schoolName}</span>
                  <span>الخطة العلاجية لمهارات القراءة والكتابة (١٢١ صفحة)</span>
